...
func Benchmark_root(b *testing.B) {
	...
	*/
	...
	handleNewgame()
	handlePosition(
		"position startpos moves d2d4 d7d5 c1f4 g8f6 e2e3 e7e6 b1d2 c7c5 c2c3 b8c6 g1f3 f8e7 f1d3 c8d7",
	)
...
func fakeRoot() {
	...
	for ix := range ml {
		...
		if score > bs {
			...
			alpha = score
			tell(
				fmt.Sprintf("info score cp %v depth %v nodes %v pv ", bs, depth, cntNodes),
				pv.String(),
			)
		}
	}
	ml.sort()
	tell(
		fmt.Sprintf("info score cp %v depth %v nodes %v pv ", bm.eval(), depth, cntNodes),
		pv.String(),
	)
...
func Test_see(t *testing.T) {
	tests := []struct {
		...
		// King
		{
			...
			C4,
			F7,
			"rnbqk2r/pppp1pp1/5n1p/2b1p1N1/2B1P3/8/PPPP1PPP/RNBQK2R w KQkq - 0 5",
			100,
		},
		...
		// Mixed
		{
			...
			D3,
			H7,
			"r2q1rk1/pp1bbppp/2n1pn2/2pp2N1/3P1B2/2PBP3/PP1N1PPP/R2QK2R w KQ - 6 9",
			-225,
		},
		{
			"Nxh7",
		...
		},
		{
			...
			D5,
			F3,
			"2kr1bnr/pb3ppp/1p2p3/2pqn3/8/P1PP1NP1/1P3PBP/RNBQ1RK1 b - - 0 10",
			-300,
		},
		...
		{"Dxg7", G4, G7, "r3kbnr/pb3ppp/1p2p3/2pqn3/3N2Q1/2PP2P1/PP3PBP/RNB2RK1 w kq - 4 3", -850},
		{
			...
			G4,
			E6,
			"r3kbnr/pb3ppp/1p2p3/2pqn3/3N2Q1/2PP2P1/PP3PBP/RNB2RK1 w kq - 4 3",
			-850,
		},
		...
		{"fxe4", F5, E4, "rnbqkbnr/ppppp2p/8/5pp1/3PP3/8/PPP2PPP/RNBQKBNR b KQkq - 0 3", 100},
		{
			...
			B5,
			C4,
			"rnbqkbnr/p1pppppp/8/1p6/2P5/8/PP1PPPPP/RNBQKBNR b KQkq - 0 2",
			100,
		},
...
func Test_initQs(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"QxQ delayed",
			"position fen r1b1kbnr/ppp2ppp/2n5/3pp1q1/3P2Q1/4PN2/PPP2PPP/RNB1KB1R w KQkq - 4 5",
			[]args{{D4, E5}, {F3, E5}, {F3, G5}},
		},
		{
			"ungarded pawn",
			"position fen rnbqkbnr/p1pppppp/8/1p6/2P5/8/PP1PPPPP/RNBQKBNR w KQkq - 0 2",
			[]args{{C4, B5}},
		},
	}
	for _, tt := range tests {
...
func Test_QS(t *testing.T) {
	tests := []struct {
		...
		{
			"",
			"After fxe4 and Bxg5 it is material equal",
			"position fen rnbqkbnr/ppppp2p/8/5pp1/3PP3/8/PPP2PPP/RNBQKBNR b KQkq - 0 3",
			50,
			10,
		},
		{
			"",
			"fxe4 and black is pawn up",
			"position fen rnbqkbnr/ppppp2p/8/5pp1/3PP3/8/PPPK1PPP/RNBQ1BNR b kq - 1 3",
			80,
			20,
		},
		{
			"",
			"equal after fxe4",
			"position fen rnbqkbnr/ppppp2p/8/5pB1/3PP3/8/PPP2PPP/RN1QKBNR b KQkq - 0 3",
			-60,
			15,
		},
		{
			"",
			"Pawn can take unguarded pawn",
			"position fen rnbqkbnr/p1pppppp/8/1p6/2P5/8/PP1PPPPP/RNBQKBNR w KQkq - 0 2",
			pieceVal[wP],
			20,
		},
		...
		{
			"",
			"Pawn captures guarded pawn",
			"position fen rnbqkbnr/ppp1pppp/8/3p4/2P5/8/PP1PPPPP/RNBQKBNR w KQkq - 0 1",
			0,
			20,
		},
		{
			"",
			"Pawn captures unguarded knight. Now under with queen and pawn",
			"position fen rnb1kbnr/ppp1pppp/8/3n4/2P5/8/PP1PPPPP/RNBQKBNR w KQkq - 0 1",
			pieceVal[wP] + pieceVal[wQ],
			20,
		},
		{
			"",
			"White is Q-P down",
			"position fen rnbqkbnr/p1pppppp/8/1p6/2P5/8/PP1PPPPP/RNB1KBNR w KQkq - 0 2",
			-pieceVal[wQ] + pieceVal[wP],
			20,
		},
		{
			"",
			"Black is Q-P down",
			"position fen rnb1kbnr/p1pppppp/8/1p6/2P5/8/PP1PPPPP/RNBQKBNR b KQkq - 0 2",
			-pieceVal[wQ] + pieceVal[wP],
			20,
		},
		...
		{
			"",
			"Pawn promotes and no captures",
			"position fen 8/1k2P3/8/q7/8/8/6K1/8 w - - 0 1",
			30,
			30,
		},
		{
			"",
			"Pawn promotes with capture. B under. This special case is not working prperly",
			"position fen 3b1n2/1k2P3/8/q7/8/8/6K1/8 w - - 0 1",
			pieceVal[wP] - pieceVal[wB] - pieceVal[wN],
			30,
		},
		...
		{
			"WN-P",
			"White is up knight-pawn",
			"position fen rnbqkbnr/ppp1pppp/8/3p4/1N6/8/PP1PPPPP/RNBQKBNR w KQkq - 0 1",
			pieceVal[wN] - pieceVal[wP],
			20,
		},
		{
			"wN+q",
			"White is up knight and queen",
			"position fen rnb1kbnr/ppp1pppp/8/3n4/5N2/8/PP1PPPPP/RNBQKBNR w KQkq - 0 1",
			pieceVal[wN] + pieceVal[wQ],
			20,
		},
		{
			"bN capt",
			"Bl Knigh captures guarded W queen",
			"position fen rnbqkbnr/ppp1pppp/8/3n4/5N2/8/PP1PPPPP/RNBQKBNR b KQkq - 0 1",
			pieceVal[wN],
			20,
		},
		// Bishop
		{
			"wB",
			"White is up bishop and queen",
			"position fen rnb1kbnr/ppp1pppp/8/3p4/2B5/8/PP1PPPPP/RNBQKBNR w KQkq - 0 1",
			pieceVal[wB] + pieceVal[wQ],
			30,
		},
		{
			"B capt N",
			"Bishop captures guarded knight",
			"position fen rnbqkbnr/ppp1pppp/8/3n4/4B3/8/PP1PPPPP/RNBQKBNR w KQkq - 0 1",
			25,
			10,
		},
		{
			...
			"position fen rnbqkbnr/ppp1pppp/8/3b4/4Q3/8/PP1PPPPP/RNBQKBNR b KQkq - 0 1",
			pieceVal[wB],
			25,
		},
		// Rook
		{
			"wR+Q",
			"White is up Rook and queen",
			"position fen rnb1kbnr/ppp1pppp/8/3p4/3R4/8/PP1PPPPP/RNBQKBNR w KQkq - 0 1",
			pieceVal[wR] + pieceVal[wQ],
			30,
		},
		{
			"wR+N",
			"White is up Rook vs knight",
			"position fen rnbqkbnr/ppp1pppp/8/3n4/3R4/8/PP1PPPPP/RNBQKBNR w KQkq - 0 1",
			pieceVal[wR] - pieceVal[wN],
			20,
		},
		{
			"bR up",
			"Black is up a Rook",
			"position fen rnbqkbnr/ppp1pppp/8/3r1B2/8/8/PP1PPPPP/RNBQKBNR b KQkq - 0 1",
			pieceVal[wR],
			25,
		},
		// Queen
		{
			"",
			"White is up a Queen vs a Pawn",
			"position fen rnbqkbnr/ppp1pppp/8/3p4/4Q3/8/PP1PPPPP/RNBQKBNR w KQkq - 0 1",
			pieceVal[wQ] - pieceVal[wP],
			20,
		},
		{
			"",
			"White is up 2 Queens",
			"position fen rnb1kbnr/ppp1pppp/8/3n4/2Q5/8/PP1PPPPP/RNBQKBNR w KQkq - 0 1",
			pieceVal[wQ] * 2,
			50,
		},
		{
			"",
			"Black Queen captures guarded W queen and is up a Queen",
			"position fen rnbqkbnr/ppp1pppp/8/3q4/4Q3/8/PP1PPPPP/RNBQKBNR b KQkq - 0 1",
			pieceVal[wQ],
			60,
		},
		// King
		{
			"",
			"White is Queen up after King captures unguarded pawn",
			"position fen rnb1kbnr/ppp1pppp/8/3p4/2K5/8/PP1PPPPP/RNBQ1BNR w KQkq - 0 1",
			pieceVal[wQ],
			60,
		},
		{
			"",
			"Black is knight up",
			"position fen rnbqkbnr/ppp1pppp/8/3n4/3K4/8/PP1PPPPP/RNBQ1BNR w KQkq - 0 1",
			-pieceVal[wN],
			60,
		},
		{
			"",
			"Bl King captures unguarded W knight. It's equal",
			"position fen rnbq1bnr/ppp1pppp/8/3k4/4N3/8/PP1PPPPP/RNBQKBNR b KQkq - 0 1",
			-45,
			20,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if val < tt.wantEval-tt.wantDelta || val > tt.wantEval+tt.wantDelta {
				t.Errorf(
					"%v:  %v Should be in [%v,  %v] %v",
					tt.name,
					val,
					tt.wantEval-tt.wantDelta,
					tt.wantEval+tt.wantDelta,
					tt.comment,
				)
			}
...
func Test_nextNormal(t *testing.T) {
	tests := []struct {
		...
		{
			"London",
			"fen rnbqk2r/pp3ppp/3bpn2/P1pp4/3P1B2/2PBP3/1P1N1PPP/R2QK1NR b KQkq - 0 8",
			"d6f4",
			"e8g8",
			"c8f5",
			[]string{"c5d4", "d6f4"},
				...
				for _, strMv := range tt.nonCapt {
					if mv.String() == strMv {
						fmt.Println(
							...
							"found good noncapt",
							mv,
							int(history.get(mv.fr(), mv.to(), b.stm)),
						)
						cntFound++
					}
				}
