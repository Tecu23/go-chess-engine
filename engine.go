...
func (e *ebfStruct) ebf(depth int) float64 {
	...
	fmt.Printf(
		"ebf: %0.2f age=%v Used=%v Stored: %v Tried: %v Found: %v Prunes: %v Best: %v\n",
		ebf,
		trans.age,
		trans.cntUsed,
		trans.cStores,
...
func search(alpha, beta, depth, ply int, pv *pvList, b *boardStruct) int {
	...
	if useTT {
		...
		if transMove, transSc, scType, ok = trans.retrieve(b.fullKey(), transDepth, ply); ok &&
			!pvNode {
			switch {
...
func nextNormal(genInfo *genInfoStruct, b *boardStruct) (move, string) {
	...
	case nextK1: // not transMove
		genInfo.sv = nextK2
		if killers[genInfo.ply].k1 != noMove &&
			...
			if b.isLegal(killers[genInfo.ply].k1) {
				...
				mv.packMove(
					...
					b.sq[killers[genInfo.ply].k1.fr()],
					b.sq[killers[genInfo.ply].k1.to()],
	...
	case nextK2: // not transMove
		...
		if killers[genInfo.ply].k2 != noMove &&
			...
			if b.isLegal(killers[genInfo.ply].k2) {
				...
				mv.packMove(
					...
					b.sq[killers[genInfo.ply].k2.fr()],
					b.sq[killers[genInfo.ply].k2.to()],
