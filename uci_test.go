...
func Test_Uci(t *testing.T) {
	...
	tests := []struct {
		...
		{"skit", "skit", []string{"info string unknown cmd skit"}},
		{
			"pos skit",
			"position skit",
			[]string{"info string Error\"skit\" must be \"fen\" or \"startpos\""},
		},
		{"position no cmd", "position", []string{"info string Error[] wrong length=1"}},
		{
			"pos incorrect move 1",
			"position startpos moves e2j4",
			[]string{"info string e2j4 in the position has an incorrect to square"},
		},
		{
			"pos incorrect move 2",
			"position startpos moves e3e4",
			[]string{"info string e3e4 in the position command. fr_sq is an empty square"},
		},
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for ix, want := range tt.wanted {
				...
				if len(want) > len(all2GUI[ix]) {
					t.Errorf(
						"%v: we want %#v (in index %v) but we got %#v",
						tt.name,
						want,
						ix,
						all2GUI[ix],
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if limits.infinite != tt.want {
				t.Errorf(
					"%v: limits.infinite should be %v but is %v",
					tt.name,
					tt.infinite,
					limits.infinite,
				)
...
func Test_handlePosition(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"fen",
			"position fen rnbqkb1r/ppp1pp1p/5np1/3p4/3P1B2/2N1P3/PPP2PPP/R2QKBNR b KQq - 1 4",
			[]arg{{A1, wR}},
			castlings(shortW | longW | longB),
		},
		{
			"startpos",
			"position startpos",
			[]arg{{A1, wR}, {A8, bR}, {E5, empty}},
			castlings(shortW | longW | shortB | longB),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
