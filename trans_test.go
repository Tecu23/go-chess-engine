...
func Test_Trans(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			3,
			0xf,
			"third entry should be replaced by fifth",
		},
		{
			A1,
			A2,
		...
		},
		{
			...
			5,
			0xf,
			"fifth entry replaces the third",
		},
	}
	...
	for ix, e := range tests {
		...
		if transMove, transScore, transScType, ok = trans.retrieve(key, depth, ply); ok {
			...
			case 2: // special case
				t.Errorf(
					"case %v: Shouldn't find its values. (mv %v, transMv %v), (sc %v, transSc %v), (st %v, transSt %v\n",
					ix+1,
					uint(mv),
					uint(transMove),
					sc,
			...
			case 4: // special case
				if transMove != mv || transScore != sc || transScType != scType {
					t.Errorf(
						"case %v: Did not overwrite case 3. (mv %v, transMv %v), (sc %v, transSc %v), (st %v, transSt %v\n",
						ix+1,
						uint(mv),
						uint(transMove),
						sc,
...
func Test_transpStruct_retrieve(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			transMv, transSc, transScType, found := tt.t.retrieve(
				tt.args.fullKey,
				tt.args.depth,
				tt.args.ply,
			)
