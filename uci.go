...
func handleMyPositions(words []string) {
	...
	default:
		tell(
			"info string not correct pos command " + words[1] + " doesn't exist. " + strings.Join(
				words[:],
				" ",
			),
		)
...
func handleMoveVal() {
	...
	genInfo := genInfoStruct{sv: 0, ply: 1, transMove: transMove}
	next = nextNormal
	ix := 0
	...
	for mv, msg := next(&genInfo, b); mv != noMove; mv, msg = next(&genInfo, b) {
		...
		fmt.Printf(
			...
			ix+1,
			mv,
			history.get(mv.fr(), mv.to(), board.stm),
			see(mv.fr(), mv.to(), &board),
			pcSqScore(mv.pc(), mv.to())-pcSqScore(mv.pc(), mv.fr()),
			msg,
		)
		ix++
	}
